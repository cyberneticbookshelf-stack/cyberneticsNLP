...
Last updated: 23 April 2026 (v0.5.1)
...
| 2026-04-18 | Cowork | Release prep — stale variable automation and source fixes (fifth batch, continued into 20 April): `src/check_stale_vars.py` created from scratch — detects and auto-fixes stale `_LDA_BASE` fallback lists across all 8 pipeline scripts using a line-scanner (not `re.sub()` — avoids backslash mangling per CLAUDE.md); cross-verifies `patch_topic_names.py` TAXONOMY; flags hardcoded corpus-count literals; first run fixed 7 scripts. `src/run_all.sh` updated: (a) automatic runlog generation via `exec > >(tee "$RUNLOG") 2>&1` with collision-safe date-based naming; (b) `check_stale_vars.py --fix` added after `patch_topic_names.py`. `figures/fig7_keyphrases.png` regenerated with 18 April canonical topic names (was showing 14 April Run C names). `data/outputs/keyphrases.html` repaired after sshfs write error (blank file); reconstructed by running `06_build_report.py` from sandbox. | v0.4.7 |
| 2026-04-20 | Cowork | Naming server permanent deployment design (third session): permanent hosting architecture agreed — `record_topic_run.py` to run 24/7 on home Linux box (a self-hosted server), which runs Apache on Docker with a DDNS client keeping the IP current. Plan: add a survey subdomain CNAME to the DDNS address in the DNS registrar; forward survey port on home router; run server under `tmux`/`systemd`; nightly cron job rsyncs `data/topic_naming.db` back to the NLP machine. Rater URL: the survey subdomain at port 7474. Eliminates SSH tunnel and ngrok dependency entirely. Setup checklist written to `CLAUDE.md` agenda item 9; status note added to `CHANGELOG.md` [0.4.7]. Implementation pending. | v0.4.7 |
| 2026-04-20 | Cowork | Naming server deployment and workflow review (second session): `src/record_topic_run.py` deployed live on the NLP machine for the first time. SSH tunnel established to the NLP machine; naming form accessed at http://localhost:7474 from the workstation. Test naming session submitted by Paul W — run_20260420_k9_s5, 9 topic names saved to `data/topic_naming.db`. Workflow fragility noted: server requires manual start on the NLP machine and SSH session held open; options for persistent/public hosting identified for next sprint (tmux/systemd, 0.0.0.0 binding, ngrok). `CHANGELOG.md` updated to add `record_topic_run.py` to [0.4.7] Added section; `CLAUDE.md` updated with naming server tunnel note and workflow fragility item; `contributions.md` updated with this entry. | v0.4.7 |
| 2026-04-20 | Cowork | Release prep — five source fixes + KI-09 resolution (fifth batch conclusion): (1) `src/03_nlp_pipeline.py` STOPWORDS — 13 contraction stems added (`aren, couldn, didn`, etc.) that were bypassing the stop list post-lemmatisation; (2) `src/06_build_report.py` cosine section — both `542 × 542` literals replaced with `{len(book_ids)} × {len(book_ids)}`; (3) `src/06_build_report.py` clusters section — interpretive caveat box added noting silhouette scores 0.013–0.021 (no valid cluster structure), exploratory-only status, and potential k variation between runs; (4) `src/run_all.sh` — `09c_validate_topics.py` moved to after `patch_topic_names.py` + `check_stale_vars.py`; (5) `src/06_build_report.py` `_PAGES` nav — `('books.html', '📝 Summaries')` replaced with `('book_nlp_entity_network.html', '🕸 Network')`. **KI-09 resolved:** `src/14_entity_network.py` — `_singular_form()` + `concept_plural_map` added; plural concept nodes merged into singulars via book-set union; `_CONCEPT_PLURAL_EXCEPTIONS` (35 terms) blocks `-ics` field names; paragraph-window `target_tls` normalised. **`src/09_extract_index.py` casing fix:** `_canonical_term()` `LOWER_IN_TITLE` set added (articles, conjunctions, prepositions legitimately lowercase in title-case proper nouns); `_ok()` updated; all-caps OCR pre-processing added (multi-word ALL-CAPS lowercased if any word > 3 chars, exempting genuine acronym sequences); "best casing wins" logic in `all_terms` vocab builder. Fixes systematic lowercasing of multi-word proper names containing English function words (e.g. `Experiments in Art and Technology`, `Laws of Form`). Rebuild from step 09 required. `CHANGELOG.md`, `contributions.md`, `CLAUDE.md` updated. | v0.4.7 |

| 2026-04-23 | Cowork | Documentation maintenance and entity network noise suppression: `CLAUDE.md` — security principle applied throughout (machine names, SSH credentials, email address, Google Form ID replaced with generic descriptions; lookup table moved to gitignored `csv/infrastructure.csv`); `README.md` — corpus framing corrected (695 collection / 541 analysed), canonical run stats updated (v0.5.0, 9/9 stable, mean stability=0.357), output file table restructured by group (book-level split, other, chapter-level not yet refactored, Excel), `books.html` excluded from release with 60k token sampling explanation, chapter-level flagged as not yet refactored throughout, step 4 summary caveat added, security fixes applied. `src/14_entity_network.py` — four fixes: (1) `NOISE_TERMS` extended with generic academic words (`work`, `project`, `study`, etc.), OCR suffix fragments (`tion`, `ence`, `tems`, etc.), and surviving function words (`this`, `there`, etc.) — confirmed noise suppression for Wiener neighbourhood; (2) `_LDA_BASE` updated to 18 April 2026 taxonomy; (3) corpus count 542 → 541 in methodological note; (4) duplicate `<div id="stats_panel">` removed. Standalone rerun of step 14: 1,604 nodes (concepts 758→722), 11,644 edges, LCC 100%, APL 3.2. | v0.5.1 |
| 2026-04-26 | Cowork | Topic naming finalisation for 25 April full-text canonical run: 9 topic names revised by Paul Wong following review of `data/outputs/topic_validation.md` against the 25 April canonical run. Substantive changes from the interim 25 April taxonomy: T4 "Systems theory & Viable Systems Model" → "Social and Organisational Cybernetics" (Beer/VSM as recursive organisational model + Luhmann's broader social-institutions scope); T9 "Ecology and posthuman technologies" → "Ecology, Posthumanism and Digital Ontology" → "Extensions of Cybernetics" (broader framing accommodates ecology, posthumanism, second-order cybernetics, digital ontology, and other extension paths); T6 "Reinvention of self and others" → "Reinventing Selves and Others, Past and Future"; T7 "Psychological regulation and control" → "Psychological and Behavioural Regulation and Control"; T1/T2/T3/T5/T8 capitalisation normalised. T9 notes rewritten to reflect broader framing. **Note:** "Extensions of Cybernetics" was the T3 name in the pre-25-April sampled-run taxonomy; same name now occupies a different topic position under the full-text canonical fit. Propagation chain executed cleanly (`patch_topic_names.py` → `check_stale_vars.py --fix` → `09c_validate_topics.py --top 10 --md`). `README.md` topic-name table (lines 350–363, pre-25-April taxonomy) replaced with pointer block referencing `json/nlp_results.json['topic_names']`, `src/patch_topic_names.py` TAXONOMY, and `data/outputs/topic_validation.md` as authoritative sources — stops README rotting on every re-naming. k=9 confirmed appropriate for full-text corpus. Run logged via `log_pipeline_run.py` as `run_20260426_k9_s5`, equivalence class `23b29233a67b2938`, nlp_hash `901e5ec924248fe2`, 2368 runlog lines ingested. Recovery from `nlp_hash` short-circuit gap diagnosed and routed to ROADMAP #28: initial logging defaulted to today's filename and missed `runlog20260425-5.csv` (canonical run started 25 April, completed 26 April 00:47 AEST); rerun with explicit `--runlog` returned "Nothing to do" because `src/log_pipeline_run.py:287` short-circuits on existing `nlp_hash` regardless of whether `runlog_entries` rows exist. Recovery via `sqlite3 DELETE` + rerun. v0.5.3 ⚑ flag cleared; `CLAUDE.md` post-run validation block removed (no longer applicable). Survey workflow now unblocked. | v0.5.3 |
| 2026-04-21 | Cowork | Google Forms survey infrastructure and pipeline.db refactor: Caddy/self-hosted approach superseded by ISP port block — pivoted to Google Forms API as permanent survey collection mechanism. Methodological design: equivalence classes for LDA runs (`run_hash = SHA-256(16)` of k/n_books/max_features/pipeline_mode/seeds_used); `nlp_hash = SHA-256(16)` of `nlp_results.json` contents to identify specific run instances. `src/pipeline_db.py` — central DB module with 8-table schema and hash utility functions. `src/log_pipeline_run.py` — manual run logging with interactive prompt, equivalence class display, sibling run listing, `--test` flag. `src/migrate_pipeline_db.py` — one-shot migration from `topic_naming.db`. `src/get_google_token.py` — one-time OAuth for machines with a browser (the workstation via sshfs mount). `src/generate_google_form.py` — fully rewritten: pre-flight enforces logged non-test run; creates 39-item Google Form with per-topic blocks (stability score, top words, stable core words, top books, name/confidence/notes fields); stores config in `google_form_configs`. `src/ingest_google_responses.py` — new: fetches paginated Google Form responses; deduplicates by `response_id`; idempotent; `--dry-run` and `--list-forms` modes. `src/record_topic_run.py` refactored to use `pipeline_db`. `src/run_all.sh` — `--test` flag; test runlog naming (`runlogYYYYMMDD_test.csv`); survey-logging reminder in completion message. OAuth troubleshooting: resolved `run_console()` removal, CSRF state error, and Error 403 (access_denied) by running `get_google_token.py` on the workstation with native browser and adding the project Google account to OAuth consent screen test users. First live form confirmed: Form ID recorded in `data/pipeline.db` (`google_form_configs` table), run `run_20260421_k9_s5`, class `0ab6e3f8ba95d0d0`. `data/pipeline.db` replaces `data/topic_naming.db`. All five documentation files updated. | v0.5.0 |
