...
Last updated: 21 April 2026 (v0.5.0)
...
| 2026-04-20 | Cowork | Naming server permanent deployment design (third session): permanent hosting architecture agreed — `record_topic_run.py` to run 24/7 on home Linux box (`cybernetic-bookshelf.org`), which runs Apache on Docker with a mywire.org DDNS client keeping the IP current. Plan: add `survey.cybernetic-bookshelf.org → CNAME → feedforward.mywire.org` in DNS registrar; forward survey port on home router; run server under `tmux`/`systemd`; nightly cron job rsyncs `data/topic_naming.db` back to Cybersonic. Rater URL: `http://survey.cybernetic-bookshelf.org:7474`. Eliminates SSH tunnel and ngrok dependency entirely. Setup checklist written to `CLAUDE.md` agenda item 9; status note added to `CHANGELOG.md` [0.4.7]. Implementation pending. | v0.4.7 |
| 2026-04-20 | Cowork | Naming server deployment and workflow review (second session): `src/record_topic_run.py` deployed live on Cybersonic for the first time. SSH tunnel established (`ssh -L 7474:localhost:7474 u9714433@cyber -N`); naming form accessed at http://localhost:7474 from AshbyX. Test naming session submitted by Paul W — run_20260420_k9_s5, 9 topic names saved to `data/topic_naming.db`. Workflow fragility noted: server requires manual start on Cybersonic and SSH session held open; options for persistent/public hosting identified for next sprint (tmux/systemd, 0.0.0.0 binding, ngrok). `CHANGELOG.md` updated to add `record_topic_run.py` to [0.4.7] Added section; `CLAUDE.md` updated with naming server tunnel note and workflow fragility item; `contributions.md` updated with this entry. | v0.4.7 |
