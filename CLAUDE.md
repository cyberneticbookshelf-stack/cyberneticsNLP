...
- **Topic names do not survive a corpus change — and this is now enforced.** Each growth mints a new equivalence class, and the clusters *recombine* rather than merely permuting: on 20 September two pairs of July topics merged, one split, one dispersed, and 0 of 9 names landed on the right topic, silently. Since 21 September `patch_topic_names.py` matches names to topics **by content, not position** — each name carries a signature (its validated top words) and a provenance record, and the script **refuses to write** if k changed, if the equivalence class changed, or if any name's best match is weak (<0.50 overlap) or ambiguous. It exits non-zero and prints the alignment plus a re-validation procedure. Verified: replaying the July taxonomy against the September run raises 7/9 failures instead of mislabelling everything.
  - `python3 src/patch_topic_names.py --report` — see the alignment, write nothing.
  - After re-validating names against a new run: edit `TAXONOMY`, then `--emit-signatures` and paste the new provenance + signature blocks.
  - **Note `check_stale_vars.py` cannot catch a naming error** — it validates scripts against `nlp_results.json`, so once a wrong name is written there it reports "9/9 match". `patch_topic_names.py` is the gate; `check_stale_vars.py` only propagates.
